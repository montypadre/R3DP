	...
	class CMB2_Bootstrap_208 {
		...
		public function include_cmb() {
			...
			if ( ! defined( 'CMB2_DIR' ) )
			{
				...
				define( 'CMB2_DIR', trailingslashit(get_template_directory()).'inc/cmb/' );
			}
		...
		public function l10ni18n() {
			...
			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'threed', '/languages/' );
			}
      //
			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'cmb2' );
				$mofile = trailingslashit( get_template_directory() ) . 'inc/cmb/languages/cmb2-' . $locale . '.mo';
