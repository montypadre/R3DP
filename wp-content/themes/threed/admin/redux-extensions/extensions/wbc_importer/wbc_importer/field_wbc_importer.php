...
if ( !class_exists( 'ReduxFramework_wbc_importer' ) ) {
    ...
    class ReduxFramework_wbc_importer {
        ...
        function __construct( $field = array(), $value ='', $parent ) {
            ...
            {
                $this->extension_dir = trailingslashit( str_replace( '\\', '/', trailingslashit( get_template_directory()).'admin/redux-extensions/extensions/wbc_importer/wbc_importer' ) );
            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'threed' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'threed' );
                    }
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'threed' ).'</span><span class="button-primary importer-button import-demo-data">' . esc_html__( 'Import Demo', 'threed' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'threed' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'threed' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'threed' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'threed' )."</h5>";
            }
