    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function __construct( $parent ) {
            ...
            if ( empty( $this->extension_dir ) ) {
                $this->extension_dir = trailingslashit( str_replace( '\\', '/', trailingslashit( get_template_directory()).'admin/redux-extensions/extensions/wbc_importer' ) );
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
                        ...
                        $primarymenu=get_terms( 'nav_menu', array( 'hide_empty' => true,'slug'=>'threed-primary-menu' ) );
                        $locations['primary'] = $primarymenu[0]->term_id;
                        set_theme_mod('nav_menu_locations', $locations);

                        /***** SETTING STATIC PAGE ****/
                        $page = get_page_by_title( 'Threed Front Page' );
                        update_option( 'page_on_front',$page->ID);
                        update_option( 'show_on_front','page');
                        wp_cache_delete ( 'alloptions', 'options' );

                    }else {
                        echo esc_html__( "Demo Already Imported", 'threed' );
                    }
                }
        ...
        public function overload_field_path( $field ) {
            ...
            return trailingslashit( get_template_directory() ).'admin/redux-extensions/extensions/wbc_importer/' . $this->field_name . '/field_' . $this->field_name . '.php';
        }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'threed' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'threed' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'It will import the demo contents and set the HOME page [ Please install the required Plugins before doing it ]', 'threed' ) ).'</p>',
