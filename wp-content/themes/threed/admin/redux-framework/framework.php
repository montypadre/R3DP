...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        ...
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_instances.php';
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_api.php';

        // General helper functions
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_helpers.php';

        // General functions
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_functions.php';
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.p.php';

        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_filesystem.php';

        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/themecheck/class.redux_themecheck.php';

        // Welcome
        require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/welcome/welcome.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( trailingslashit( get_template_directory() ).'admin/redux-framework/' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'threed' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'threed' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'threed' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'threed' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'threed'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'threed' );
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    load_plugin_textdomain( 'threed', '', self::$_dir . 'languages/' . $locale . '.mo' );
                } else {
                    load_textdomain( 'threed', self::$_dir . 'languages/' . $locale . '.mo' );
                }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( trailingslashit( get_template_directory() ).'admin/redux-framework/inc/tracking.php' ) ) {
                    require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/tracking.php';
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'threed' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'threed' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'threed' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'threed' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'threed' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'threed' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'threed' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = trailingslashit( get_template_directory() ).'admin/redux-framework/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'threed' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'threed' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'threed' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
